...
class Payment {
  constructor({ appid, mchid, partnerKey, pfx, notify_url, refund_url, spbill_create_ip, sandbox, accounts, corpId, corpAgentId, corpSecret } = {}, debug = false) {
    ...
    // this.pfx = pfx;
    this.pfx = require('fs').readFileSync(pfx);
    ...
    this.urls = {
      ...
      querycontract: '/papay/querycontract',
      ...
      getWxpayfaceAuthinfo: '/face/get_wxpayface_authinfo',
      facepay: '/pay/facepay',
      sendworkwxredpack: '/mmpaymkttransfers/sendworkwxredpack',
      queryworkwxredpack: '/mmpaymkttransfers/queryworkwxredpack',
      paywwsptrans2pocket: '/mmpaymkttransfers/promotion/paywwsptrans2pocket',
      querywwsptrans2pocket: '/mmpaymkttransfers/promotion/querywwsptrans2pocket',
    };
    this.debug = debug;
  }
  ...
  async _parse(xml, type) {
    ...
    switch (type) {
      ...
      case 'middleware_nativePay':
      case 'getsignkey':
      case 'getWxpayfaceAuthinfo':
        break;
      default:
        if (json.return_code !== 'SUCCESS') throw new Error(json.err_code || 'XMLDataError');
    }

    switch (type) {
      case 'middleware_contract':
      case 'contract':
        //免密签约
        break;
      case 'getsignkey':
  ...
  _getSign(params, type = 'MD5') {
    ...
    console.log('_getSign', str, util.md5(str).toUpperCase())
    switch (type) {
      case 'MD5':
  ...
  async _request(params, type, cert = false, prefix = '') {
    // 安全签名
    ...
    this.log('---post data:---\r\n%s\r\n', pkg.data);
    let tmpPrefix = this.prefix;
    if (prefix) {
      tmpPrefix = prefix
    }
    let { status, data } = await urllib.request(tmpPrefix + this.urls[type], pkg);
  ...
  sendworkwxredpack (params) {
    ...
    pkg['workwx_sign'] = this.qwsign(pkg,1)
    return this._request(pkg, 'sendworkwxredpack', true);
  }
  // 查询企业红包
  queryworkwxredpack (mch_billno) {
    let pkg = {
      appid: this.corpId,
      mch_id: this.mchid,
      nonce_str: util.generate(),
    ...
    return this._request(pkg, 'queryworkwxredpack', true);
  }
  // 向员工付款
  paywwsptrans2pocket (params) {
    let pkg = {
      ...params,
      appid: this.corpId,
      mch_id: this.mchid,
    ...
    pkg['workwx_sign'] = this.qwsign(pkg, 2)
    return this._request(pkg, 'paywwsptrans2pocket', true);
  }
  // 查询付款记录
  querywwsptrans2pocket (partner_trade_no) {
    let pkg = {
      appid: this.corpId,
      mch_id: this.mchid,
      nonce_str: util.generate(),
    ...
    return this._request(pkg, 'querywwsptrans2pocket', true);
  }
  // 企业微信签名1
  qwsign(params, type =1) {
    let inc = []
    if(type ==1) {
    /**
     * 发红包api固定如下几个字段参与签名：
      ...
      wxappid
    */
      inc = ['act_name', 'mch_billno', 'mch_id', 'nonce_str', 're_openid', 'total_amount','wxappid']
    } else if(type == 2) {
      /**
       * 付款api固定如下几个字段参与签名：
        amount
        ...
        ww_msg_type
       */
      inc = ['amount', 'appid', 'desc', 'mch_id', 'nonce_str', 'openid', 'partner_trade_no','ww_msg_type']
    }
    let newParams = {}
    inc.map(i => {
      newParams[i] = params[i]
    })
    let str = util.toQueryString(newParams) + '&secret=' + this.corpSecret;
    console.log('_getSign', str, util.md5(str).toUpperCase())
    return util.md5(str).toUpperCase()
  }
}
